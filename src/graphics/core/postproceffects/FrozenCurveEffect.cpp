...
FrozenCurveEffect::FrozenCurveEffect(const Config& config) : PostProcEffect(config), 
    shader(config.shaders_dir + "postproc/postproc_frozencurveeffect.vert",
    config.shaders_dir + "postproc/postproc_frozencurveeffect.frag")
{
...
GLuint FrozenCurveEffect::render(GLuint input_color_buffer, uint32_t frame)
{
    if (width == 0)
    {
